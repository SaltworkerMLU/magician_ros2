...
from python_qt_binding.QtWidgets import QGraphicsLineItem, QGraphicsScene
...
class DobotControlPanel(QWidget):
    ...
    def __init__(self, node, plugin=None):
        ...
        _, package_path = get_resource('packages', 'rqt_2D')
        ui_file = os.path.join(package_path, 'share', 'rqt_2D', 'resource', 'DobotControlPanel_layout.ui')
        loadUi(ui_file, self)

        # Timer for updating the line position
        self._latest_joint_msg = None
        self._line_update_timer = QTimer(self)
        self._line_update_timer.timeout.connect(self._on_line_update_timer)
        self._line_update_timer.start(50)  # update every 50 ms
        ...
        bg_image_path = os.path.join(package_path, 'share', 'rqt_2D', 'resource', 'images', 'dobot_magician_2D.png')
        bg_pixmap = QPixmap(bg_image_path)

        # Get the size of the graphicsView widget
        ...
        self.scene.setSceneRect(0, 0, view_size.width()-10, view_size.height()-10)
        self.scene.setBackgroundBrush(QBrush(scaled_pixmap))

        # Draw initial line
        self.link_2_start = [160, 250]
        self.link_len = [0, 135 * 0.66, 147 * 0.66]

        self.link_2 = QGraphicsLineItem(self.link_2_start[0], self.link_2_start[1], 
                                        self.link_2_start[0], self.link_2_start[1] - self.link_len[1])
        self.link_3 = QGraphicsLineItem(self.link_2_start[0], self.link_2_start[1] - self.link_len[1], 
                                        self.link_2_start[0] + self.link_len[2], self.link_2_start[1] - self.link_len[1])
        self.link_2.setPen(QPen(Qt.blue, 5))
        self.link_3.setPen(QPen(Qt.red, 5))
        self.scene.addItem(self.link_2)
        self.scene.addItem(self.link_3)
    ...
    def _on_line_update_timer(self):
        if self._latest_joint_msg is not None:
            self.update_line_position(self._latest_joint_msg)

    def update_line_position(self, msg):
        self.link_2.setLine(self.link_2_start[0], 
                            self.link_2_start[1], 
                            self.link_2_start[0] + self.link_len[1]*math.sin(msg.position[1]),    
                            self.link_2_start[1] - self.link_len[1]*math.cos(msg.position[1])
                            )
        self.link_3.setLine(self.link_2_start[0] + self.link_len[1]*math.sin(msg.position[1]), 
                            self.link_2_start[1] - self.link_len[1]*math.cos(msg.position[1]),
                            self.link_2_start[0] + self.link_len[1]*math.sin(msg.position[1]) + self.link_len[2]*math.cos(msg.position[2]),
                            self.link_2_start[1] - self.link_len[1]*math.cos(msg.position[1]) + self.link_len[2]*math.sin(msg.position[2]))
    ...
    def joints_positions_callback(self, msg):
        ...
        self.JT2LCDv2.setText(str(round((math.degrees(msg.position[1])), 3)))
        self.JT3LCDv2.setText(str(round((math.degrees(msg.position[2])), 3)))
